...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedMimes.includes(file.mimetype.toLowerCase())) {
      ...
      cb(
        new Error(
          `Unsupported image format: ${file.mimetype}. Please use JPEG, PNG, GIF, BMP, WebP, or TIFF.`
        ),
        false
      );
    }
...
app.get("/test-ai", async (req, res) => {
  try {
    ...
    for (const testModel of testModels) {
      ...
      if (
        found &&
        found.supportedGenerationMethods?.includes("generateContent")
      ) {
...
app.post("/process-image", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Image info: ${req.file.mimetype}, size: ${req.file.size} bytes`
    );
    ...
    if (error.message.includes("OCR processing failed")) {
      res.status(400).json({
        ...
        details:
          "The image may be unclear, corrupted, or contain no readable text. Please try a different image.",
        suggestion:
      ...
      res.status(500).json({
        ...
        details:
          "There was an issue processing your text with AI. Please try again.",
        suggestion:
          "If the problem persists, try with a shorter text or different image.",
      });
    } else {
      res.status(500).json({
